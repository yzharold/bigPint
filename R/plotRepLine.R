...
#' @importFrom dplyr %>%
#' @importFrom shiny need hr fluidPage fluidRow column selectInput selectizeInput numericInput actionButton br div p img addResourcePath
#' @importFrom shinydashboard dashboardSidebar sidebarMenu dashboardBody tabItem tabItems box dashboardHeader
#' @importFrom Hmisc cut2
#' @importFrom RColorBrewer brewer.pal
#' @importFrom ggplot2 labs scale_fill_manual coord_fixed
#' @export
#' @examples
#' data(soybean_ir)
#' soybean_ir[,-1] <- log(soybean_ir[,-1]+1)
#' data(soybean_ir_metrics)
#' plotRepLine(soybean_ir, soybean_ir_metrics)
plotRepLine = function(data, dataMetrics){
  
  countColor2 <- counts <- hexID <- ID <- key <- val <- Sample <- Count <- NULL
  dat <- data
  ...
  values <- shiny::reactiveValues(x=0, selPair=NULL, selMetric=NULL, selOrder=NULL)
  ...
  body <- dashboardBody(
    ...
    column(width = 8,
      ...
      div(img(src='www/Figure1.PNG', style="width: 75%; height: 75%"), style="text-align: center;"),
      br(),
      br(),
      div(p('Figure 2'), style="text-align: center;"),
      div(img(src='www/Figure2.PNG', style="width: 75%; height: 75%"), style="text-align: center;"),
      br(),
      br(),
      div(p('Figure 3'), style="text-align: center;"),
      div(img(src='www/Figure3.PNG', style="width: 75%; height: 75%"), style="text-align: center;")
      )))
  ...
  server <- function(input, output, session) {
    ...
    #addResourcePath("www", "/Users/lindz/bigPint/inst/www")
    addResourcePath("www", paste0(system.file(package="bigPint"), "/www"))
